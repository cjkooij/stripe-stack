...
import { formatUnixDate, hasUnixDateExpired } from '~/utils/date'
...
export async function loader({ request }: DataFunctionArgs) {
  ...
  // Redirect with the intent to setup user subscription.
  if (!subscription) return redirect('/resources/stripe/create-subscription')

  // Redirect with the intent to update session.
  // User is subscribed but session is not updated.
  if (subscription.planId && !user.subscription?.planId) {
    return redirect('/resources/user/update-session')
  }

  // Redirect with the intent to update session.
  // Subscription has expired.
  if (hasUnixDateExpired(subscription.currentPeriodEnd)) {
    return redirect('/resources/user/update-session')
  }
...
export default function Account() {
  ...
  // Our user data, includes current subscription.
