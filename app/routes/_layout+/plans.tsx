...
export default function Plans() {
  const { user, defaultCurrency } = useLoaderData<typeof loader>()
  const [plansInterval, setPlansInterval] = useState<Interval | string>(
    user?.subscription?.interval || Interval.MONTH,
  )
    ...
    <div className="flex w-full flex-col items-center justify-start px-6 md:h-full">
      ...
      <div className="my-4 flex flex-col items-center justify-center">
        <div className="text-center font-bold text-gray-200">
          {plansInterval === Interval.MONTH ? 'Monthly' : 'Yearly'}
        </div>
        <div className="my-2" />
        ...
        <label htmlFor="toggle" className="flex cursor-pointer items-center">
          <div className="relative">
            <input
              ...
              value=""
              className="sr-only"
              checked={plansInterval === Interval.YEAR}
              onChange={() =>
                setPlansInterval((prev) =>
          ...
          return (
            <div
              ...
              className={`mx-2 flex min-w-[280px] flex-col items-center px-6 py-3 transition hover:opacity-100 ${
                user && user.subscription?.planId === plan.id
                  ? 'opacity-100'
                  : 'opacity-40'
              }`}>
              {/* Thumbnail. */}
              ...
              <h5 className="flex flex-row items-center text-5xl font-bold text-gray-200">
                ...
                <small className="relative top-2 left-1 text-lg text-gray-400">
                  {plansInterval === Interval.MONTH ? '/mo' : '/yr'}
                </small>
              </h5>
              <div className="my-3" />
